...
module.exports.Bot = (function() {
  ...
  var onStanza = function(stanza) {
    ...
    var body, fromJid, fromChannel, fromNick, x;
    ...
    if (stanza.is('message') && stanza.attrs.type === 'groupchat') {
    ...
    } else if (stanza.is('presence') && !stanza.attrs.type) {
      // HipChat handles presence information with presence stanzas which look
      // like this:
      //
      // <presence to='22555_416155@chat.hipchat.com/hubot-hipchat'
      //     from='22555_data_science_stand_up@conf.hipchat.com/Greeves McBot'>
      //   <x xmlns='http://jabber.org/protocol/muc#user'>
      //     <item affiliation='member'
      //       jid='22555_416155@chat.hipchat.com/hubot-hipchat'
      //       role='participant'/>
      //     <status code='110'/>
      //   </x>
      // </presence>
      //
      //  The DTD is here: http://xmpp.org/schemas/muc-user.xsd
      ...
      // Participants are HipChat users, visitors are guests using the HipChat
      // web UI to join public channels without a HipChat account
      if (role !== 'participant' && role !== 'visitor') {
        return;
      }
      ...
      // If the user has gone away store their presence and return, we use this
      // to differentiate between a user returning from away and a user actually
      // joining the channel
      if (show && show.getText() === 'away') {
        this.presences[fromJid.toString()] = 'away';

        return;
      }

      // If the user is coming back from away delete their presence and return
      if (this.presences[fromJid.toString()] === 'away') {
        delete this.presences[fromJid.toString()];

        return;
      }

      this.emit('join', fromChannel, fromNick, role);
    }
  };
  ...
  // Emitted whenever a user joins a channel the bot is in.
  //
  // `onJoin(callback)`
  //
  // - `callback`: Function to be triggered: `function (channel, user, role)`
  //   - `channel`: The name of the room which was joined.
  //   - `user`: The name of the person who joined the channel.
  //   - `role`: The role of the person who joined the channel.
  Bot.prototype.onJoin = function (callback) {
    this.on('join', function (channel, user, role) {
      callback(channel, user, role);
    });
  };
